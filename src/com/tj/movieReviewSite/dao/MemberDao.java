...
public class MemberDao {
	...
	public int join_member(MemberDto member) {
		...
		String sql="INSERT INTO MEMBER (MID,MPW,MNAME,MBIRTH,MEMAIL,MTEL,MADDRESS)" + 
				"    VALUES (?,?,?,?,?,?,?)";
		try {
			conn= getConnection();
	...
	//--로그인
	public int login_chk(String mId, String mPw) {
		int result = LOGIN_FAIL;
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String sql="SELECT * FROM MEMBER WHERE MID=? AND MPW=? AND MMOD=0";
		try {
			conn= getConnection();
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, mId);
	...
	public MemberDto getMember(String mId) {
		...
		try {
			...
			if(rs.next()) {
				 ...
				 int 	mMod 		= rs.getInt("mMod");
				 member = new MemberDto(mId, mPw, mName, mBirth, mEmail, mTel, mAddress, mRdate, mMod);
			}
		} catch (SQLException e) {
			System.out.println(e.getMessage());
		}finally {
	...
	public ArrayList<MemberDto> MemberList(int startRow, int endRow){
		...
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String sql="SELECT * FROM (SELECT ROWNUM RN, A.* FROM (SELECT * FROM MEMBER WHERE MMOD=0 ORDER BY MRDATE DESC)A)" + 
		...
		try {
			...
			while(rs.next()) {
				 ...
				 int 	mMod 		= rs.getInt("mMod");
				 member.add(new MemberDto(mId, mPw, mName, mBirth, mEmail, mTel, mAddress, mRdate, mMod));
			}
		} catch (SQLException e) {
			System.out.println(e.getMessage());
		}finally {
	...
	public int delete_member(String mId,String mPw) {
		int result = FAIL;
		Connection conn = null; 
		PreparedStatement pstmt = null;
		String sql="UPDATE MEMBER SET MMOD=1 WHERE MID=? AND MPW=?";
		try {
			conn = getConnection();
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, mId);
