...
public class FreeBoardDao {
	...
	public int insert_freeBoard(String mId, String bTitle,String bContent) {
		int result = FAIL;
		Connection conn = null; 
		PreparedStatement pstmt = null;
	...
	public FreeBoardDto getFreeBoard(int bNo) {
		...
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String sql="SELECT F.*,MNAME FROM FREEBOARD F,MEMBER M WHERE F.MID=M.MID AND BNO=?";
		try {
			conn = getConnection();
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, bNo);
	...
	public int reply_freeBoard(String mId,String bTitle,String bContent,int bGroup,int bStep,int bIndent) {
		stepA_freeBoard(bGroup, bStep);
		int result = FAIL;
		Connection conn = null; 
		PreparedStatement pstmt = null;
		String sql="INSERT INTO FREEBOARD (BNO,MID,BTITLE,BCONTENT,BGROUP,BSTEP,BINDENT)" + 
